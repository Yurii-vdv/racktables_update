...
/********************************************
 *
 * RackTables 0.22.x snmpgeneric extension v0.2
 *
 * needs PHP 5 >= 5.4.0 (SNMP Class)
 *testing Ubuntu 24.10 and PHP 8.3 
 *
...
$sg_cmd_snmptranslate = '/usr/bin/snmptranslate';
...
$sg_ifType2oif_id = array (
	...
	'209' => 31,	/* bridge => virtual port 1469 */
	'23' => 2003,	/* 3g-sim => virtual port 2000 */
);
...
function snmpgeneric_getsnmp ($snmpconfig, &$snmpdev)
{
	...
	foreach ($systemoids as $shortoid => $oid)
	{
		...
		if ($shortoid == 'sysUpTime')
		{
			...
			$mins = (int)($secs / (60));
			$secs -= $mins * 60;
			$value = "$value ($days d $hours h:$mins:$secs)";
		}
...
//array(1) { [0]=> string(31) ".1.3.6.1.2.1.47.1.1.1.1.5.65536" }
	//if (0)
	if (!empty ($chassis))
	{
		echo '<table>';
		...
		foreach ($chassis as $key => $oid)
		{
			...
			if (!preg_match ('/\.(\d+)$/',$oid, $matches))
				continue;
//var_dump($matches);//array(2) { [0]=> string(6) ".65536" [1]=> string(5) "65536" }
			$index = $matches[1];

            $sysDescr = $snmp->get  (".1.3.6.1.2.1.1.1.0");                     
			$cpuname = $snmp->get (".1.3.6.1.2.1.47.1.1.1.1.7.$index");
			$serialnum = $snmp->get (".1.3.6.1.4.1.14988.1.1.7.3.0");
			//$serialnum = $snmp->get (".1.3.6.1.2.1.47.1.1.1.1.11.$index");
			$mfgname = $snmp->get (".1.3.6.1.2.1.47.1.1.1.1.12.$index");
			$modelname = $snmp->get (".1.3.6.1.2.1.47.1.1.1.1.2.$index");
			//$modelname = $snmp->get (".1.3.6.1.2.1.47.1.1.1.1.13.$index");

			showSuccess ("CPU $cpuname $mfgname $modelname s/n: $serialnum");

			//echo ("<tr><td>$cpuname</td><td>$mfgname</td><td>$modelname</td><td>$serialnum</td>");
		}
		unset ($key);
		unset ($oid);
...
{ global $sg_known_sysObjectIDs,$sysDescr;
	
	$attr = &$sysObjectID['attr'][$attr_id];

				
//SELECT * FROM `Dictionary` WHERE `dict_value` LIKE '%RB433AH%'
$value = $sg_known_sysObjectIDs[$sysDescr]["dict_key"];
...
if ($script_mode)
{
	...
	if ($sg_dry_run)
		echo "Dry-run Mode! No changes mode!\n";
...
function snmpgeneric_process (&$data, &$object, &$snmpdev)
{
	...
	//$sysobjid  = string(7) "14988.1"RB 433AH
