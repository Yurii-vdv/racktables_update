...
function pl_layout_port($port, $number, $pos)
{
	...
	if($port['linked'])
		$details .= "<td>Remote:<br>".$port['cableid']."<br>".$port['remote_object_name']."<br>".$port['remote_name']."<div id=\"port${port_id}-status-detail-remote\">".$title."</div></td>";
...
//echo $ipv4;
	...
	$ifipv4 = $object['iftable'][$port_name]['ipv4'];
...
class sl_ifxsnmp extends SNMP
{
	...
	function getiftable()
	{
		...
		$oid_ifmac =  '.1.3.6.1.2.1.2.2.1.6'; //ifXtable
		$oid_ifname_object = '.1.3.6.1.2.1.1.1.0';
		$oid_ifipv4 = '.1.3.6.1.2.1.4.20.1.2';
		...
		$ifmac = $this->walk($oid_ifmac, TRUE); //iftable
		$ifipv4 = $this->walk($oid_ifipv4, TRUE); //iftable
...
//count interface ipaddress
$count_ip = count(array_keys($ifipv4, $index));
if($count_ip != 0)
{//var_dump(array_keys($ifipv4, $index));
$i = 0;
while ($i <= $count_ip) {
    $list_ip = array_keys($ifipv4, $index);
    $res_list_ip .= ", ".$list_ip[$i];
   
    $i++;  
}
 $retval[$ifname[$index]]['ipv4'] = $res_list_ip;
}
